...
import {
  ...
  runAgent,
  type SpawnFn,
} from '../../../afk-runner/src/agent-backend/agent-runner.js'
import type { ProgressReporter } from '../../../afk-runner/src/agent-backend/progress-log.js'
import { cleanupTempDirs, makeTempDir } from './test-helpers.js'

const ReviewerIssuesSchema = z.object({ issues: z.array(z.unknown()) })
const FixerResultSchema = z.object({
  verdict: z.string(),
  fixability: z.string(),
  reasoning: z.string(),
...
describe('runAgent backend integration', () => {
  test('the bare options compose today’s opencode argv: no stdin, no env', async () => {
