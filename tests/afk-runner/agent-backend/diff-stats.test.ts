...
import {
  ...
  touchedTestPath,
  type ExecGitFn,
} from '../../../afk-runner/src/agent-backend/diff-stats.js'
...
describe('touchedTestPath', () => {
  ...
  test('agrees with the repo TDD resolver rather than inventing a second rule', () => {
    // Expected booleans recorded from .hooks/tdd/test-resolver.mjs's isTestFile at port time.
    const cases: Array<[string, boolean]> = [
      ['tests/review-loop/a.test.ts', true],
      ['tests/a.spec.tsx', true],
      ['src/a.ts', false],
      ['review-loop/src/a.ts', false],
      ['a.test.js', true],
      ['notatest.ts', false],
      ['a.test.ts.bak', false],
      ['src/a.spec.jsx', true],
    ...
    for (const [p, expected] of cases) {
      expect(touchedTestPath([p])).toBe(expected)
    }
  })
})
