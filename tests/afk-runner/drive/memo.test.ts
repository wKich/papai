...
describe('state.json is a derived memo, never truth (design D6)', () => {
  it('a converged run drives the S tail; the R1 auto-approve completes it and the memo comes from the fold', async () => {
    ...
    const memo = PersistedRunStateSchema.parse(JSON.parse(fs.readFileSync(memoPath, 'utf8')))
  ...
  it('a stale memo is overwritten from the fold on the next status/resume', async () => {
    ...
    const runDir = pipeline.runDirOf(started.runId)
    const memoPath = path.join(runDir, 'state.json')
    const stale = PersistedRunStateSchema.parse(JSON.parse(fs.readFileSync(memoPath, 'utf8')))
    stale['depth'] = 'L'
    stale['round'] = 99
    ...
    const rewritten = PersistedRunStateSchema.parse(JSON.parse(fs.readFileSync(memoPath, 'utf8')))
