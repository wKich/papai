...
describe('runReviewLoop — raised vs open', () => {
  ...
  async function runOnce(
    ...
    mutate = true,
  ): Promise<{ result: ReturnType<typeof runReviewLoop> extends Promise<infer R> ? R : never; fixture: LoopFixture }> {
  ...
  it('writes the round hash snapshot before the round_close event', async () => {
    const dir = makeDir()
    const { fixture } = await runOnce(dir, {
      reviewer: [JSON.stringify({ findings: [finding({ class: 'MATERIAL' })] })],
      resolver: [editingResolver('MATERIAL')],
    })
    expect(fs.existsSync(path.join(dir, 'sidecars', 'round-hashes-1.json'))).toBe(true)
    const closeIndex = fixture.emitted.findIndex((e) => e.type === 'round_close')
    const convergenceIndex = fixture.emitted.findIndex((e) => e.type === 'convergence')
    expect(closeIndex).toBeGreaterThan(convergenceIndex)
  })

  it('treats an edit that moved nothing as still open once a prior snapshot exists', async () => {
    // Round 1 has no prior snapshot, so its claim is taken at face value; round 2
    // compares against round 1's snapshot and catches a static change folder.
    const dir = makeDir()
    const fixture = makeLoopFixture(dir, {
      reviewer: [JSON.stringify({ findings: [finding({ class: 'MATERIAL' })] })],
      resolver: [editingResolver('MATERIAL')],
