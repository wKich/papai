...
`auto_decision.decision` widened additively with `'pending'` (deadline-waiter re-arm/stay-pending records); pre-widening logs parse unchanged.

The loop-memory fields are additive the same way: a `finding` event carries an optional `fingerprint` (the classified emission stamps it, joining the event to its cross-round concern), and a `convergence` event carries optional `concerns: string[]` (the thrash cluster ids, present only on the round that ended the loop as a concern-history cap-hit). Pre-change logs replay to `concerns: []` and otherwise byte-identical state.
...
On an unmetered run the unknown marker no longer forces the gate (see Config and autonomy) — it renders for reporting while the round cap and the R2 bound carry the spend risk.
...
The two lenses' findings are merged by a normalized gap **fingerprint** (case-folded, punctuation-stripped, stopword-pruned token set — `fingerprintOf` in `work/concern-model.ts`), not exact text; differently-quoted copies of one concern merge to a single finding carrying the most severe class. Skeptic ids are namespaced (`S1, S2, …`, enforced by the skeptic spawn's sidecar refine), and a resolver sidecar with duplicate finding ids fails validation and retries like any schema violation. Convergence counts **distinct merged findings** — duplicate entries can no longer inflate counts or flip verdicts.
...
**Concern-history gate.** When a concern with ≥2 prior resolved/dismissed entries is raised again, or a resolved concern is re-raised at a different class, the loop stops instead of silently running another round: the convergence event carries the cluster ids (`concerns`), and whichever gate follows — early or final — carries a `### Concern history` section listing each recurring concern's round-by-round history.

**Cross-artifact consistency check.** At every round the lens merge is appended with a deterministic scan (`work/artifact-consistency.ts`) over the change folder's proposal/design/spec files: seeded decision-term vocabulary (migration strategy, `*60*1000` interval literals, backticked table/column identifiers) rendered differently across artifacts becomes a synthesized `C<n>` MATERIAL finding naming both files and both renderings, resolved by the resolver like any finding. No spawn, no tokens; the vocabulary widens only on analyzer evidence. Synthesized findings carry fingerprints like any other, so a disagreement the resolver dismisses twice ends the loop as thrash on the third raise.
...
Five keys, single mode: `repoRoot`, `workDir`, `model`, `budget` (USD, default 5; `null` = unmetered), `deadline` (minutes, optional), plus the optional `metered` boolean (derived: true unless `budget` is `null`). Everything else is rejected at load time naming the replacement (`autonomy`/`budgetUsd` → `budget`, `models` → `model`, `timeouts` → compiled constants). The ladder always evaluates and settles what it can (assist semantics): level ranks, the `rules` map, `permittedAt`, and the R2 auto-extend count bound are gone — the trajectory window and the R4 budget guard against the one `budget` key are the sole extension bounds. Audit records are unconditional: every presented gate gets its preview block, one `auto-policy.jsonl` line, and an event; undecidable gates additionally append the workdir-level `policy-debt.jsonl` entry.

**Unmetered runs** (`budget: null`, or an explicit `metered: false`): subscription providers report `costUsd: 0`, repricing falls through to unknown, and R4's cost-unknown branch used to fire first on every gate — the ladder could never decide. On an unmetered run that branch is disabled: the round cap plus the R2 trajectory predicate are the sole spend guards. The explicit-exceedance branch predicates on numeric-ceiling presence, so an explicitly configured numeric budget is never bypassed by anything (`metered: false` + `budget: 5` keeps the ceiling and drops only the cost-unknown veto).

The deterministic decision ladder (`sdd-runner/src/auto-policy.ts`, pure): **R1** converged-final-approve (0 open BLOCKER/MATERIAL/NITPICK + all surviving assumptions low-blast → auto-approve); **R2** trajectory auto-extend (cap-hit, 0 BLOCKERs, ≥1 open MATERIAL, strictly decreasing open findings over the last 2 rounds, projected spend under `budget` → auto-extend exactly one round reusing `runExtendRound`; the bound is persisted before the extended round spends); **R3** assumption blast-radius triage (low-blast iff all `evidence.files` recorded, inside the change folder/run dir, no spec delta, no `tasks.md` line; missing/empty/un-cross-checkable evidence fails closed to high-blast; the agent's `blast_radius` text is display-only; on a mixed gate R3 pre-checks the low-blast items with `decided-by: policy R3` and still presents the gate); **R4** budget guard (metered run with `costKnown === false`, or projected exceedance against a numeric `budget` → gate; the cost-unknown branch is metered-only); **R5** reversibility (leaving-the-branch actions are never auto-decided).
...
The config `deadline` key (minutes) arms `gateDeadlineAt` at presentation (bell/notification line; the process exits without blocking). The derived wait: a deadline is set and the context is non-TTY ⇒ the waiter runs. It polls `state.json` and the gate file each second without caching, settles stable hand edits through the normal path (content hash unchanged 3 consecutive ticks), translates landing steer directives, and at expiry claims the gate through the same first-writer-wins settle claim as the TUI, re-runs the ladder conservatively over the run's **real** gate state (sidecar review result, replayed trajectory, gathered cost signals — the same `evaluateLadder` the prelude uses, metered R4 treatment included; R1 approve, else R2 extend, else stay pending), and re-arms at most once — never auto-aborts. Every claimed expiry outcome appends the standard `auto_decision` event after its write (settle names the deciding rule; re-arm/stay-pending record `none`/`pending`), so replay alone distinguishes waiter-settled gates from human-settled ones; externally-settled gates (hand edit, steer) emit nothing.
