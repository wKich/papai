...
| codeindex CI experiment | [`docs/operations/codeindex-ci-experiment.md`](docs/operations/codeindex-ci-experiment.md)     | opencode-agent job's variable-gated codeindex provisioning (sibling checkout, prebuild, canary), per-job usage report, keep/revert rubric, single-variable reversibility                                                                                                                                                                                                                                                                                                                                            |
...
| SDD pipeline            | [`docs/architecture/sdd-pipeline.md`](docs/architecture/sdd-pipeline.md)                       | stages, event model, depth profiles, gate protocol (process sections canonical; the `sdd-runner/` workspace was deleted at R5 — `afk-runner/` implements the pipeline)                                                                                                                                                                                                                                                                                                                                              |
| afk-runner              | [`docs/architecture/afk-runner.md`](docs/architecture/afk-runner.md)                           | graph kernel over xstate pure transition, fold-is-truth engine, grow-not-restore plan (C1–C9 delivered incl. three live-proof cycles — C9 armed both budget regimes through the execution half), living U-ledger (re-scored after C9; next: U13 the post-plan e2e weak-point audit), closed relaxation window (re-tighten performed at R5), runner commands incl. the read-only `analyze` corpus report (the sole runner since R5 deleted the sdd-runner fallback; rollback is `git revert` of the deletion commit) |
| afk-runner MCP research | [`docs/architecture/afk-runner-mcp-research.md`](docs/architecture/afk-runner-mcp-research.md) | injection surfaces through the one spawn seam (merge-vs-override verified live), scoping, server catalogue, credentials/degradation doctrine, ranked recommendation + follow-ups (issue #403)                                                                                                                                                                                                                                                                                                                       |
...
See `tests/CLAUDE.md`. Prefer DI over `mock.module()` where the module supports it. Helpers (`schemaValidates()`, `getToolExecutor()`, `setMockFetch()`, `restoreFetch()`) live in `tests/utils/test-helpers.ts`; `tests/mock-reset.ts` resets common mocks per test. The repo mixes DI-first and legacy delayed-import/mock suites — follow the local pattern unless intentionally refactoring style. Mutation testing (Stryker) runs as a blocking per-file ratchet gate in CI but is not in the write-hook pipeline. It measures **product code only** — `src/`, `client/`, `plugins/`, `review-loop/src/`, `afk-runner/src/` and `opencode-agent/src/` (that last tree's `index.ts` barrel excluded), via `isGateableImplFile`, the same predicate the write hook uses; `scripts/`, `mutation-improve/` and every workspace path outside those roots (the rest of `opencode-agent/` included) keep their `tests/` suites but get no per-file floor, so a branch touching only those selects zero targets and passes. `scripts/mutation/baseline.json` is the monotonic per-file floor — per-file records `{score, killed, timeout, scored}`, with bare legacy score numbers still coexisting during the lazy migration — seeded on master from changed files via `test:mutate:changed --base=HEAD~1 --update-baseline` / `seedMerge`. The ratchet fails a baselined file only when its score drops below the record AND its kills (`killed + timeout`) fall below the recorded ones — a true regression; a below-floor score with kills held is new-code dilution and prints a `WARN` instead of failing. A score-only legacy entry is judged by score alone. On PRs the gate runs as **plan → k × shard → gate** (`test:mutate:plan` / `:shard` / `:gate`): the matrix is sized from estimated work, so a small PR stays on one shard and never spawns one, and the gate fails when any planned target has no result — a lost shard is a failure, never a narrower verdict. `test:mutate:changed` remains the single-process local command. The PR gate judges the **whole branch diff** every run while only **measuring** files whose content changed since the previous run; the rest carry over fingerprint-guarded scores, so a regression from an earlier commit keeps failing later pushes. `--no-score-cache` forces a full re-measure.
...
| Autonomous SDD pipeline (`/sdd:auto`)           | `docs/architecture/afk-runner.md`; use `/sdd:auto <task-file>` for end-to-end runs                                                                              |
