...
| afk-runner             | [`docs/architecture/afk-runner.md`](docs/architecture/afk-runner.md)                         | graph kernel over xstate pure transition, fold-is-truth engine, grow-not-restore plan (C1–C8 delivered incl. two live-proof cycles), living U-ledger (re-scored after C8; next: U3), closed relaxation window (re-tighten performed at R5), runner commands incl. the read-only `analyze` corpus report (the sole runner since R5 deleted the sdd-runner fallback; rollback is `git revert` of the deletion commit) |
