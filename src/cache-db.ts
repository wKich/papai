...
export function syncHistoryToDb(userId: number, messages: unknown[]): void {
  try {
    ...
    log.error({ userId, error: error instanceof Error ? error.message : String(error) }, 'Failed to sync history to DB')
  }
}
...
export function syncSummaryToDb(userId: number, summary: string): void {
  try {
    ...
    log.error({ userId, error: error instanceof Error ? error.message : String(error) }, 'Failed to sync summary to DB')
  }
}
