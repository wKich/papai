...
/**
 * Report rendering (D8): plain text — no ANSI escapes, analysis output is
...
function gatesLine(run: RunAnalysis): string {
  return metricLine(run.gates, (gates): string => {
    ...
    const waiterBits: string[] = []
    if (gates.waiterPendingRecords > 0) waiterBits.push(`pending ×${gates.waiterPendingRecords}`)
    if (gates.waiterRearms > 0) waiterBits.push(`re-arms ×${gates.waiterRearms}`)
    if (waiterBits.length > 0) parts.push(`waiter: ${waiterBits.join(' · ')}`)
...
/** Fixed cause order; every cause renders its nonzero count, ` · ` separators, omitted when none. */
function r2CauseSuffix(byCause: R2CauseMix): string {
  const parts: string[] = []
  const causes: readonly R2Cause[] = ['r2-fired', 'cost-unknown', 'over-ceiling', 'preview', 'trajectory-blocked']
...
function mixLine(mix: Readonly<Record<string, number>>): string {
  return Object.entries(mix)
    .map(([key, count]) => `${key} ${count}`)
    .join(' · ')
}

function stageFailuresLine(run: RunAnalysis): string {
  return metricLine(
    run.stageFailures,
    (byStage) =>
      Object.entries(byStage)
        .map(([stage, kinds]) => `${stage}: ${mixLine(kinds)}`)
        .join(' · ') || 'none',
  )
}
...
function usageLine(run: RunAnalysis): string {
  ...
  const bound = run.usage.unpricedEvents > 0 ? ` ≥ $${total.toFixed(2)} (${run.usage.unpricedEvents} unpriced)` : ''
  return `  usage: ${detail.length === 0 ? 'none' : detail} — $${total.toFixed(2)}${bound} (${known})`
}
...
function consistencyLines(run: RunAnalysis): readonly string[] {
  ...
  if (audit.memoDivergingFields.length > 0) {
    flags.push(`stale memo (diverging fields: ${audit.memoDivergingFields.join(', ')})`)
  }
...
function runSection(run: RunAnalysis): readonly string[] {
  const identity = [run.changeName ?? 'unnamed', run.status ?? 'no memo'].join(' · ')
  const lines = [
  ...
  lines.push(`  resolver mix: ${metricLine(run.resolverActionMix, mixLine)}`)
  ...
  lines.push(
    `  retries: ${metricLine(run.retries, (byRole) => {
      const parts = Object.entries(byRole).flatMap(([role, counts]) => [
  ...
  lines.push(`  stage failures: ${stageFailuresLine(run)}`)
