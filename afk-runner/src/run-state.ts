...
/**
 * Slimmed Tier 2 copy (C3): the round-cap table lives here so the review-model
 * copy can import it without the legacy review-loop cycle.
 */
 ...
 * the caller's sink; the cap re-read consults the persisted `roundCap`
 * so a steered `extend` takes effect at the next boundary.
 */
